...
export default function Contact() {
  ...
  const submit = async (e: React.FormEvent) => {
    e.preventDefault()
    await fetch('https://formspree.io/f/xkokkker', {
      method: 'POST',
  ...
  return (
    <section id="contact" style={{ padding: '5rem 1.5rem', maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '4rem', alignItems: 'start' }}>
        <div>
          <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--blue)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>Contact</p>
          <h2 style={{ fontSize: 'clamp(26px, 4vw, 36px)', letterSpacing: '-0.02em', lineHeight: 1.2, marginBottom: '1.25rem' }}>Let's get your business online.</h2>
          <p style={{ fontSize: 15, color: 'var(--text-muted)', lineHeight: 1.7, marginBottom: '2rem' }}>
            Fill out the form and we'll get back to you within a few hours — usually the same day. No commitment, no pressure.
          </p>
        ...
        <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '2rem' }}>
          {submitted ? (
            <div style={{ textAlign: 'center', padding: '2rem 0' }}>
              <div style={{ width: 48, height: 48, background: 'var(--blue-light)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 1rem' }}>
                <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
                  <polyline points="4,11 9,16 18,6" stroke="var(--blue)" strokeWidth="2" strokeLinecap="round"/>
                </svg>
              </div>
              <h3 style={{ fontSize: 22, marginBottom: '0.5rem' }}>Message sent!</h3>
            ...
            <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
              ...
              <div>
                ...
                <textarea name="message" required value={form.message} onChange={handle} placeholder="Tell us about your business and what you're looking for..." rows={4} style={{ ...inputStyle, resize: 'vertical' }} />
              </div>
              <button type="submit" style={{ background: 'var(--blue)', color: '#fff', padding: '12px', borderRadius: 'var(--radius-sm)', fontSize: 15, fontWeight: 500, border: 'none' }}>
                Send message
              </button>
              <p style={{ fontSize: 12, color: 'var(--text-faint)', textAlign: 'center' }}>We typically respond within a few hours.</p>
            </form>
          )}
        </div>
      </div>
